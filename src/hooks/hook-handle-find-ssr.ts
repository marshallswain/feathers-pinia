...
export const handleFindSsr = (Model: any) => async (context: HookContext, next: NextFunction) => {
  if (context.method === 'find') {
    ...
    if (pageData?.ssr) {
      context.result = {
        data: pageData.ids.map((id: Id) => Model.store.getFromStore(id)),
